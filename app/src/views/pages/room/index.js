...
import InviteRoomDialog from "./components/invite_room_dialog";
...
function CodeScreen({ username }) {
   ...
   const [isShowInviteDialog, setShowInviteDialog] = useState(false);
   ...
   function changeEditorLanguage(rawLanguageCode) {
      ...
      const languageCode =
         rawLanguageCode === "nodejs" ? "javascript" : rawLanguageCode.replace(/[0-9]/g, "");
   ...
   //                     <h3>Output</h3>
   //                     <Box
   //                        className={output === "" ? "result-banner" : "active-result-banner"}
   ...
   const messageTab = () => {
      return (
         <>
            <Box sx={{ border: "solid 1px #333", borderRadius: "10px", height: "100%" }}>
               ...
               <Grid container spacing={1} sx={{ marginBottom: "5px", padding: "0px 5px" }}>
                  <Grid item xs={10}>
                     <Input
                        placeholder="message..."
                        referance={inputRef}
   ...
   const getTab = () => {
      if (currentTab === 1) return memberTab();
      if (currentTab === 2) return messageTab();
      return outputTab();
   };
   ...
   function setDefaultCallingBarPosition() {
      setCommunicateBoxPosition({
         ...
         y:
            editorUIRef.current?.getBoundingClientRect().bottom -
               communicateBoxRef.current?.clientHeight -
               EDITOR_BOUND_PADDING ?? EDITOR_BOUND_PADDING,
      });
   }
   ...
   return (
      <>
         <Grid container>
            ...
            >
               ...
               <DialogContent>
                  <DialogContentText id="alert-dialog-description">
                     Do you want to replace with the new language template?
                  </DialogContentText>
               </DialogContent>
               <DialogActions>
                  <Button
            ...
            <Grid item xs={7} ref={editorUIRef}>
               <Box
                  sx={{
                     display: "flex",
               ...
               >
                  ...
                  >
                     ...
                     <Autocomplete
                        ...
                        onChange={handleOnLanguageVersionChange}
                        value={versionList.current[selectedVersionIndex] ?? ""}
                        renderInput={(params) => <TextField {...params} label="ver" />}
                     />
                  </Box>
                  <Box
                     sx={{
...
<IconButton
                     ...
                     >
                        {!camState ? (
                           <VideocamOffRounded fontSize="medium" />
                        ) : (
                           <VideocamRounded fontSize="medium" />
                        )}
                     </IconButton>
                     <IconButton
                        sx={{
                        ...
                        onClick={turnOnOffMicrophone}
                     >
                        {!micState ? (
                           <MicOffRounded fontSize="medium" />
                        ) : (
                           <MicRounded fontSize="medium" />
                        )}
                     </IconButton>
               ...
               <MainAvatarBox
                             ...
                             name="You"
                             isSpeaking={
                                !socket.current ? false : speakingUsers.includes(socket.current.id)
                             }
                             color={
                                !socket.current ? USER_DEFAULT_COLOR : userColors[socket.current.id]
                             }
                             height={AVATAR_BOX_HEIGHT}
                             width={AVATAR_BOX_WIDTH}
                             videoRef={userVideo}
                       ...
                       >
                          ...
                          <Box
                             sx={{
                                ...
                                width:
                                   (AVATAR_BOX_WIDTH + AVATAR_BOX_SPACING) *
                                   (peers.length < maxAvatarShow ? peers.length : maxAvatarShow),
                                overflow: "hidden",
                                borderRadius: 2,
                             }}
                          >
           ...
           <Grid item xs={3}>
              ...
              >
                 <BasicTabs
                    ...
                    labels={["Info", "Message"]}
                    components={[infoTab(), messageTab()]}
                    dotState={isDotInvisible}
                    setDotState={setDotInvisible}
                 />
              </Box>
           </Grid> */}
         </Grid>
